...
library(corrplot);library(MPV);library(olsrr);library(ggpubr);library(lmtest);library(car)
```
...
```
Cleaning the data means that we had to convert some values into numbers for it to work. The data for married status had to be converted into 0 for not married and 1 for married. For gender status, 0 for Female, and 1 for Male. Also, for the smoke column, Y is 1 and 0 is No, similarly to the married data. Data of the columns had to be changed to a double value to work with.
...
- 9.44 for chldbear and marriage is pretty high.
- 
...
- Looks like ther is a multicollinearity problem with weight, height, and BMI.
- gender and chldbear seems to have a VIF about 5~ and could pose as a problem.
...
hist(x=bp_full$residuals, main = paste("Density of ", "residuals") ,xlab='Residuals')
#augment(bp_full)
...
The residuals values of  look normally distributed. 
...
hist(x=bp_full_transformed$residuals, xlab='Residuals', main= paste("Density of Residuals for transformed full model"))
#augment(bp_full)
...
```{r}
#AIC=1938.49
bp_reduced <- lm(sbp_full_transformed ~ gender + smoke + exercise + height + overwt + 
    alcohol + trt + chldbear, data=bp_train)
summary(bp_reduced)
```
- we know that gender and chldbear are strongly correlated, thus, it would be good to remove them

 
```{r}
bp_reduced2 <-  lm(sbp_full_transformed ~ chldbear + smoke + exercise + height + alcohol + trt + bmi , data=bp_train)
summary(bp_reduced2)
```
We drop gender, and see that the chldbear p-value > 0.929 therefore, we remove it.



```{r}
bp_reduced3 <-  lm(sbp_full_transformed ~ smoke + exercise + height + alcohol + trt + bmi , data=bp_train)
summary(bp_reduced3)
stepAIC(bp_reduced3)
```
Checking if removing the terms is better using ANOVA: 
```{r}
anova(bp_reduced3, bp_full_transformed)
```
...
```{r}
qqnorm(bp_reduced3$residuals)
qqline(bp_reduced3$residuals)
hist(x=bp_reduced3$residuals, xlab='Residuals', main= paste("Density of Residuals for transformed reduced model"))
#augment(bp_full)
ggplot(bp_reduced3, aes(x=.fitted, y=.resid)) + geom_point() + labs(x="Fitted", y="Residuals")+ geom_hline(yintercept = 0)
bc_reduced <- boxcox(bp_reduced3)
shapiro.test(bp_reduced3$residuals)
bptest(bp_reduced3, studentize = F)
```

- bp test, p-value < 0.05 => error constancy.
...
bp_reduced_t <- lm(data=bp_train, sbp_reduced_t~smoke + exercise + height + alcohol + trt + bmi)
...
```{r}
qqnorm(bp_reduced_t$residuals)
qqline(bp_reduced_t$residuals)
hist(x=bp_reduced_t$residuals, xlab='Residuals', main= paste("Density of Residuals for transformed full model"))
#augment(bp_full)
ggplot(bp_reduced_t, aes(x=.fitted, y=.resid)) + geom_point() + labs(x="Fitted", y="Residuals")+ geom_hline(yintercept = 0)
bc_reduced2 <- boxcox(bp_reduced_t)
shapiro.test(bp_reduced_t$residuals)
bptest(bp_reduced_t, studentize = F)
```
```{r}
bp_train %>% mutate(resid = rstandard(bp_reduced_t)) -> bp_train

bp_train  %>% dplyr::select(c(resid, smoke , exercise , height , alcohol , trt , bmi)) %>% 
  pivot_longer(c (smoke , exercise , height , alcohol , trt , bmi), names_to = "xnames", values_to = "x") %>%
  ggplot(aes(x=x, y=resid))  + geom_point() + facet_wrap(~xnames, scales = "free") + geom_smooth(method="lm", se=F)
```
# Case 3
```{r}
bp_train %>% mutate(s.hat = abs(bp_reduced_t$residuals)) -> bp_train
fit = lm(s.hat~ smoke + exercise + height + alcohol + 
    trt + bmi, data=bp_train)
var.s1 = predict(fit)^2
mod1 <- lm(sbp_reduced_t ~ smoke + exercise + height + alcohol + 
    trt + bmi, data = bp_train, weight=1/var.s1)


y.hat <- predict(bp_reduced_t); e.1  <- resid(bp_reduced_t)

mod2 <- lm(abs(e.1)~y.hat)

var.s = (mod2$fitted.values)^2
mod3 = lm(sbp_reduced_t ~ smoke + exercise + height + alcohol + 
    trt + bmi, data = bp_train, weight=1/var.s)
...
mod5 = lm(sbp_reduced_t ~ smoke + exercise + height + alcohol + 
    trt + bmi, data = bp_train, weight=1/var.s2)
...
mod7 = lm(sbp_reduced_t ~ smoke + exercise + height + alcohol + 
    trt + bmi, data = bp_train, weight=1/var.s3)

e.4 = mod7$residuals
yhat.4 = predict(mod7)
mod8 = lm(abs(e.4)~yhat.4)
var.s4 = (mod8$fitted.values)^2
mod9 = lm(sbp_reduced_t ~ smoke + exercise + height + alcohol + 
    trt + bmi, data = bp_train, weight=1/var.s4)

e.5 = mod9$residuals
yhat.5 = predict(mod9)
mod10 = lm(abs(e.5)~yhat.5)
var.s5 = (mod10$fitted.values)^2
mod11 = lm(sbp_reduced_t ~ smoke + exercise + height + alcohol + 
    trt + bmi, data = bp_train, weight=1/var.s5)


e.6 = mod11$residuals
yhat.6 = predict(mod11)
mod12 = lm(abs(e.6)~yhat.6)
var.s6 = (mod12$fitted.values)^2
mod13 = lm(sbp_reduced_t ~ smoke + exercise + height + alcohol + 
    trt + bmi, data = bp_train, weight=1/var.s6)

cbind(coefficients(fit),coefficients(mod3),coefficients(mod5), coefficients(mod7),coefficients(mod9),coefficients(mod11), coefficients(mod13))
bptest(mod9)
```

```{r}
e.7 = mod13$residuals
yhat.7 = predict(mod13)
mod14 = lm(abs(e.7)~yhat.7)
var.s7 = (mod14$fitted.values)^2
mod15 = lm(sbp_reduced_t ~ smoke + exercise + height + alcohol + 
    trt + bmi, data = bp_train, weight=1/var.s7)

e.8= mod13$residuals
yhat.8 = predict(mod15)
mod16 = lm(abs(e.8)~yhat.8)
var.s8 = (mod16$fitted.values)^2
mod17 = lm(sbp_reduced_t ~ smoke + exercise + height + alcohol + 
    trt + bmi, data = bp_train, weight=1/var.s8)
...
```{r}
bp_reduced_t2 <- lm(sbp_reduced_t ~ smoke + exercise + height + alcohol  + bmi, data = bp_train)

bptest(bp_reduced_t2, studentize = F)
anova(bp_reduced_t, bp_reduced_t2)
```
...
p2 <- ols_plot_cooksd_chart(mod7)
p3 <- ols_plot_dffits(mod7)
p4 <- ols_plot_resid_lev(mod7)
p5 <- ols_plot_resid_stud_fit(mod7)
ols_plot_dfbetas(mod7)
```
## Removing outliers
```{r}
#83, 155 , 293, 135, 261, 300, 35, 25, 256, 201, 292, 242, 263

t = rstudent(bp_reduced3)
#round(t,2)
n = 300
p.prime = 7
alpha = 0.05
...
which(abs(t) > t.crit)
bp_train %>% filter(!row_number() %in% c(263) )  -> outliers_rm
bp_reduced4 <- lm(sbp_full_transformed~smoke + exercise + height + alcohol + trt + bmi, data=outliers_rm)
```


```{r}
hii = hatvalues(bp_reduced_t)
which(hii > 2*7/300)
```


```{r}
p7 <- ols_plot_cooksd_chart(bp_reduced4)
p8 <- ols_plot_dffits(bp_reduced4)
p9 <- ols_plot_resid_lev(bp_reduced4)
p10 <- ols_plot_resid_stud_fit(bp_reduced4)
bptest(bp_reduced4)
```
...
#bp_valid
preds_cols <- c( 'smoke' , 'exercise' , 'height', 'alcohol' , 'trt' ,'bmi'  )
pred_bp <- predict(mod9, bp_valid[preds_cols])
delta_bp <- bp_valid['sbp_reduced_t'] - pred_bp
n.star <- dim(bp_valid)[1]
MSPR <- sum((delta_bp)^2)/200
MSPR
MSE <- sum(mod9$residuals^2)/200
MSE
```
...
delta_bp <- bp_valid['sbp_reduced_t'] - pred_bp
n.star <- dim(bp_valid)[1]
MSPR <- sum((delta_bp)^2)/n.star
MSPR
MSE <- sum(bp_reduced_t$residuals^2)/300
MSE
```
...
```{r}
stepAIC(mod9, direction="backward")
```

```{r}
stepAIC(bp_reduced_t, direction="backward")
```
...
bp_valid %>% mutate(sbp_reduced_t = k1_r*(sbp^lambda_reduced - 1)) -> bp_valid
preds_cols <- c( 'smoke' , 'exercise' , 'height', 'alcohol' , 'trt' ,'bmi'  )
pred_bp <- predict(bp_full_transformed, bp_valid[preds_cols])
delta_bp <- bp_valid['sbp_reduced_t'] - pred_bp
n.star <- dim(bp_valid)[1]
MSPR <- sum((delta_bp)^2)/n.star
MSPR
MSE <- sum(bp_full_transformed$residuals^2)/300
MSE
```
