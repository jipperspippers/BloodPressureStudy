...
summary(bp_reduced)
```
- since gender has a large p value and in


 
```{r}
bp_reduced2 <-  lm(sbp ~ chldbear + smoke + exercise + height + alcohol + trt + bmi , data=bp_train)
summary(bp_reduced2)
```
...
```{r}
bp_reduced3 <-  lm(sbp ~ smoke + exercise + height + alcohol + trt + bmi , data=bp_train)
summary(bp_reduced3)
stepAIC(bp_reduced3)
```
- Everything looks significant here.
- AIC = 1940.93


Checking if removing the terms is better using ANOVA: 
```{r}
anova(bp_reduced3, bp_full)
```
...
pred_bp <- predict(bp_reduced3, bp_valid[preds_cols])
...
```{r}
bp.rstandard <- rstandard(bp_reduced3)
bp.rstudent <- rstudent(bp_reduced3)
bp.inf <- influence.measures(bp_reduced3)
...
p2 <- ols_plot_cooksd_chart(bp_reduced3)
p3 <- ols_plot_dffits(bp_reduced3)
p4 <- ols_plot_resid_lev(bp_reduced3)
p5 <- ols_plot_resid_stud_fit(bp_reduced3)
p6 <- ols_plot_dfbetas(bp_reduced3)
```
